  ...
  return (
    <div className="flex items-start gap-3.5">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 text-[11px] font-medium text-slate-400">{t('planRequests.requestedBy', {}, 'Asked')} {formatInstant(request.createdAt)}</p>
      </div>
...
function Decision({ pending, disabled, onApprove, onDecline, approveLabel }) {
  const { t } = useT();
  const label = approveLabel ?? t('planRequests.approveRequest', {}, 'Approve');
  ...
  return (
    <div className="grid grid-cols-2 gap-2 pt-1">
      ...
      >
        {t('planRequests.rejectRequest', {}, 'Decline')}
      </button>
    </div>
  );
}
...
export function QuantityRequest({ request }) {
  ...
  const { pending, modal, setModal, resolve, removed } = useResolver(resolveQuantityRequest, {
    approved: t('common.saved', {}, 'Approved for that day.'),
    rejected: t('common.rejected', {}, 'Declined.'),
  });
    ...
    <>
      <article className="group relative overflow-hidden rounded-3xl border-2 border-slate-200 bg-white p-5 shadow-sm transition-all hover:border-blue-400 hover:shadow-md">
        ...
        <div className="mt-4 flex items-center justify-between gap-3 rounded-2xl bg-slate-50 border border-slate-200/80 px-4 py-3">
          <div>
            <p className="font-heading text-[10px] font-extrabold uppercase tracking-wider text-slate-400">{t('planRequests.currentQty', {}, 'Regular')}</p>
          ...
          <div className="text-right">
            <p className="font-heading text-[10px] font-extrabold uppercase tracking-wider text-blue-600">{t('planRequests.requestedQty', {}, 'Requested')}</p>
        ...
        <div className="mt-3.5 space-y-3">
          ...
          <p className="text-[11px] font-medium text-slate-400">
            {t('dashboard.todayModNotice', {}, 'Affects this single day only — their ongoing daily plan remains unchanged.')}
          </p>
          <Decision
            pending={pending}
      ...
      <DeclineModal
        ...
        pending={pending}
        title={`${t('planRequests.rejectRequest', {}, 'Decline')} ${request.customerName}?`}
...
export function PlanChangeRequest({ request }) {
  ...
  const { pending, modal, setModal, resolve, removed } = useResolver(resolvePlanChangeRequest, {
    approved: t('common.saved', {}, 'Plan changed from tomorrow.'),
    rejected: t('common.rejected', {}, 'Declined.'),
  });
  ...
  const SLOT_LABEL = {
    ...
    BOTH: t('common.slot', {}, 'Morning & evening'),
  };
  ...
  return (
    <>
      <article className="card-surface relative overflow-hidden p-4 sm:p-5">
        ...
        >
          {t('subscriptions.changePlanRequest', {}, 'Wants to change plan')}
        </RequestHeader>

        <div className="mt-4 grid grid-cols-2 gap-2 sm:gap-3">
          <div className="rounded-2xl border border-border bg-surface-muted/50 p-3">
            <p className="text-[11px] font-bold uppercase tracking-wide text-ink-subtle">{t('planRequests.currentQty', {}, 'Now')}</p>
            ...
            {nowPaise > 0 ? (
              <p className="tnum text-[11px] font-semibold text-ink-subtle">{formatPaise(nowPaise, { whole: true })}/{t('subscriptions.perMonth', {}, 'mo')}</p>
            ) : null}
          </div>
          <div className={cn('rounded-2xl border p-3', retired ? 'border-critical/30 bg-critical-soft/40' : 'border-brand/40 bg-brand-soft/60')}>
            <p className={cn('text-[11px] font-bold uppercase tracking-wide', retired ? 'text-critical' : 'text-brand')}>{t('planRequests.requestedQty', {}, 'Wants')}</p>
            ...
            <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5">
              {wantsPaise > 0 ? (
                <p className="tnum text-[11px] font-semibold text-ink-subtle">{formatPaise(wantsPaise, { whole: true })}/{t('subscriptions.perMonth', {}, 'mo')}</p>
              ) : null}
        ...
        <div className="mt-3 space-y-3">
          <CustomerNote note={request.customerNote} />
          {retired ? (
            <Notice tone="critical" title={t('common.warning', {}, 'That plan is retired')}>
              {t('common.inactive', {}, 'You cannot approve a change onto a plan you have retired.')}
            </Notice>
          ) : null}
          <p className="text-xs font-medium text-ink-subtle">
            {t('subscriptions.changePlanDesc', {}, "Takes effect tomorrow. Days already delivered this month keep today's price.")}
          </p>
          <Decision
            pending={pending}
            disabled={retired}
            onApprove={() => resolve(request.id, true)}
            onDecline={() => setModal(true)}
            approveLabel={t('planRequests.approveRequest', {}, 'Approve change')}
          />
        </div>
      </article>
...
function DeclineModal({ open, onClose, pending, title, onSubmit }) {
  ...
  return (
    ...
    >
      ...
      >
        <p className="text-sm text-ink-muted">{t('customers.subtitle', {}, 'Your customer will see this.')}</p>
        <Textarea name="note" label={`${t('planRequests.reason', {}, 'Reason')} (${t('common.optional', {}, 'optional')})`} maxLength={500} />
      </form>
    </Modal>
  );
}
